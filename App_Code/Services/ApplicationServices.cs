...
namespace IXport.Services
{
    ...
    public class ApplicationServicesBase
    {
        ...
        {
            IXport.Services.ApplicationServices.Initialize();
        }
        ...
        public static void RegisterStandardMembershipAccounts()
        {
            ...
            if (Membership.GetUser("admin") == null)
            {
                ...
                admin = Membership.CreateUser("admin", "admin123%", "admin@IXport.com", "ASP.NET", "Code OnTime", true, out status);
                user = Membership.CreateUser("user", "user123%", "user@IXport.com", "ASP.NET", "Code OnTime", true, out status);
        ...
        public static void RegisterCssLinks(Page p)
        {
            foreach (Control c in p.Header.Controls)
            	if (c is HtmlLink)
                {
                    ...
                    if (l.ID == "IXportTheme")
                    	return;
                    if (l.Href.Contains("_Theme_Belltown.css"))
                    {
                        l.ID = "IXportTheme";
