...
namespace IXport.Handlers
{
    ...
    public class SiteBase : IXport.Web.PageBase
    {
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            if (_isTouchUI)
            {
                if (_pageFooterContent != null)
                	_pageFooterContent.Text = "<footer style=\"display:none\"><small><center>I-Xport - QuoSoftware - 2016</center>" +
                        "<hr></small></footer>";
            }
            else
        ...
        protected virtual void InitializeSiteMaster()
        {
            ...
            string themePath = Server.MapPath("~/App_Themes/IXport");
            if (Directory.Exists(themePath))
            	foreach (string stylesheetFileName in Directory.GetFiles(themePath, "*.css"))
                {
                    ...
                    if (!(fileName.Equals("_Theme_Belltown.css")))
                    {
                        HtmlLink link = new HtmlLink();
                        link.Href = ("~/App_Themes/IXport/" + fileName);
