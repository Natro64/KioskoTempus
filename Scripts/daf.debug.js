...
(function () {
    ...
    _app.isIE6 = this._ie6 = Sys.Browser.agent == Sys.Browser.InternetExplorer && Sys.Browser.version < 7;
    ...
    _app.prototype = {
        ...
        _confirm: function (context, confirmCallback) {
            ...
            if (m && !(__tf != 4)) {
                ...
                if (dataView && !dataView.get_isModal()) {
                    ...
                    dataView = _app.showModal(this, controller, view, commandName, commandArgument, this.get_baseUrl(), this.get_servicePath(), [], { confirmContext: context, showSearchBar: this.get_showSearchBar() });
                    result = false;
                }
            }
            else {
        ...
        executeReport: function (args) {
            ...
            $get('aa', f).value = _app.htmlEncode(Sys.Serialization.JavaScriptSerializer.serialize(aa));
        ...
        _renderItem: function (sb, field, row, isSelected, isInlineForm, isFirstRow, headerOnly, trimLongWords, templateMode) {
            ...
            if (isEditing && isSelected && !readOnly) {
                ...
                if (this._lastCommandName == 'BatchEdit')
                    sb.appendFormat('<div class="BatchSelect"><table cellpadding="0" cellspacing="0"><tr><td><input type="checkbox" id="{0}$BatchSelect{1}" onclick="_app._updateBatchSelectStatus(this,{3})"{4}/></td><td><label for="{0}$BatchSelect{1}">{2}</a></td></tr></table></div>',
        ...
        _renderSearchBarField: function (sb, settings, visibleIndex) {
            ...
            if (isFilter) {
                ...
                fsb.appendFormat('<table class="FilterValues{3}" cellpadding="0" cellspacing="0" onmouseover="_app.highlightFilterValues(this,true,\'Active\')" onmouseout="_app.highlightFilterValues(this,false,\'Active\')"><tr><td class="Values" valign="top"><div><a class="Link" onclick="$find(\'{0}\')._showSearchBarFilter({1},{2});return false;" tabindex="{4}" href="javascript:" onfocus="_app.highlightFilterValues(this,true,\'Focused\')" onblur="_app.highlightFilterValues(this,false,\'Focused\')" title="{5}">', this.get_id(), field.Index, visibleIndex, hasValues ? '' : ' Empty', $nextTabIndex(), Web.DataViewResources.Data.Filters.Labels.FilterToolTip);
                if (hasValues) {
                ...
                fsb.appendFormat('</a></div></td><td class="Button{5}" valign="top"><a href="javascript:" onclick="$find(\'{0}\')._showSearchBarFilter({1},{2});return false" title="{3}" tabindex="{4}" onfocus="_app.highlightFilterValues(this,true,\'Focused\')" onblur="_app.highlightFilterValues(this,false,\'Focused\')" >&nbsp;</a></td></tr></table>', this.get_id(), hasValues ? -1 : field.Index, visibleIndex, hasValues ? Web.DataViewResources.Data.Filters.Labels.Clear : Web.DataViewResources.Data.Filters.Labels.FilterToolTip, $nextTabIndex(), hasValues ? ' Clear' : '');
                button = fsb.toString();
            }
        ...
        dataSignature: function () {
            ...
            return Sys.Serialization.JavaScriptSerializer.serialize(filter);
        },
        ...
        _onExecuteComplete: function (result, context) {
            ...
            if (pendingUploads && pendingUploads.length > 0 && noErrors && lastCommandName.match(/^(Insert|Update)$/)/* && result.RowsAffected == 1*/) {
                ...
                key = that._findKeyValue(result.Values);
                if (key == null)
        ...
        _busy: function (isBusy) {
            ...
            if (isBusy && !interval) {
                ...
                var $busy = that._$busy = $(String.format('<div class="dataview-busy-indicator{0}"></div>', atWindowTop ? ' modal' : ''))
                    .css({ position: mobile ? 'fixed' : 'absolute', width: w, 'z-index': atWindowTop ? (mobile ? 5000 : 3000000) : 0 }).appendTo(document.body),
                    x = -w,
        ...
        _prepareJavaScriptExpressionEx: function (script) {
            return script.replace(/\[((\w+)\.)?(\w+)\]/g, 'this.fieldValue("$3","$2")')
                .replace(/\$row\.(\w+)/g, 'this.fieldValue("$1")')
                .replace(/\$master\.(\w+)/g, 'this.fieldValue("$1","master")');
        },
    ...
    _app.toServerDateTime = function (d) {
        return d != null && d.getMonth ? new Date(d - d.getTimezoneOffset() * 60 * 1000) : d;
    }

    _app.execute = function (args) {
        var placeholder = $('<div/>').appendTo($('body')).hide(),
            dataView = $create(Web.DataView, { controller: args.controller, viewId: args.view || 'grid1', servicePath: __servicePath, baseUrl: __baseUrl, useCase: '$app', tags: args.tags }, null, null, placeholder.get(0));
        ...
        dataView._collectFieldValues = function () {
            ...
            $(args.values).each(function () {
                var v = this,
                    oldValue = _app.toServerDateTime(typeof v.oldValue == 'undefined' ? v.value : v.oldValue),
                    newValue = _app.toServerDateTime(typeof v.newValue == 'undefined' ? oldValue : v.newValue);
                values.push({
                    Name: v.name || v.field,
                    NewValue: newValue,
                    OldValue: oldValue,
    ...
    */
    ...
    _window._field_text = function (v, trim) {
        ...
        if (fieldItems.length == 0)
            s = String.isBlank(v) ? nullValue : that.format(v);
    ...
    function compileBusinessRule(script) {
        var compiledScript = [],
            index = 0,
            iterator = /\$(row|master)\s*\.\s*(\w+)(\s*=([\s\S]+?)\s*;)?/g,
        ...
        while (m) {
            ...
            if (expression)
                ...
                compiledScript.push(String.format('this.fieldValue("{0}","{1}")', fieldName, scope));
    ...
    _web.BusinessRules.prototype = {
        ...
        process: function (phase) {
            ...
            if (this._newValues) {
                if ($app.mobile)
                    dataView.extension().afterCalculate(this._newValues);
                else
            ...
            $(files).each(function (index) {
                ...
                if (uploadSupport.fileReader && file.type.match(/^image\//i)) {
                    ...
                    reader.onload = function (event) {
                        $('<img/>').attr('src', event.target.result).insertAfter(this._fileInfo);
                        this._fileInfo = null;
                        if (--imageFiles == 0) {
